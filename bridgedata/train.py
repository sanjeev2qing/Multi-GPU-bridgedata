...
from bridgedata.utils.general_utils import move_to_device
from bridgedata.utils.general_utils import map_recursive
from bridgedata.utils.general_utils import AverageMeter, RecursiveAverageMeter
from bridgedata.utils.general_utils import AttrDict
from bridgedata.utils.checkpointer import CheckpointHandler
from bridgedata.utils.tensorboard_logger import Logger
from bridgedata.models.gcbc_transfer import GCBCTransfer
from bridgedata.models import get_model_class
from bridgedata.data_sets import get_dataset_class

from bridgedata.models.utils.compute_dataset_normalization import compute_dataset_normalization
from bridgedata.utils.general_utils import sorted_nicely
import shutil
...
def make_path(exp_dir, conf_path, prefix, make_new_dir):
    ...
    if conf_path.endswith('.json'):
        ...
        path = conf_path.split('bridgedata_experiments/', 1)[1]
        if make_new_dir:
            prefix += datetime_str()
...
class ModelTrainer(Configurable):
    ...
    def run_stage(self, args, stage):
        ...
        if self._hp.dataset_normalization:
                self.model.module.set_normalizing_params(self.norm_params)
                self.model_val.module.set_normalizing_params(self.norm_params)
        ...
        self._hp.mpar =  self.model.module._hp
        save_config({'train._hp': self._hp, 'model_conf':  self.model.module._hp, 'data_conf': self.data_conf},
        ...
        if stage.startswith('finetuning') and not args.resume:  # if resuming finetuning skip the mainstage
            incompatible_keys = self.model.module.load_state_dict(trained_model_state_dict, strict=False)
    ...
    def make_additional_valdatasets(self, stage):
        ...
        for key in val_keys:
            ...
            loader = DataLoader(dataset, sampler=train_sampler,batch_size=self._hp.batch_size, num_workers=8,
                          drop_last=True)
    ...
    def make_model_and_dataset(self, logger, ModelClass, DatasetClass, data_conf, phase, stage):
        ...
        # model.to(device)
        # print(device)
        model = nn.DataParallel(model,device_ids=[args.local_rank],output_device=0)
        ...
        if phase is not 'test':
            ...
            loader = DataLoader(dataset, sampler=self.train_sampler, batch_size=self._hp.batch_size, num_workers=8,
                                drop_last=True)
    ...
    def train_epoch(self, epoch, num_epochs):
        ...
        self.model.module.set_dataset_sufix(self.train_dataset._hp)
        ...
        for self.batch_idx, sample_batched in enumerate(self.train_loader):
            ...
            losses = self.model.module.loss(inputs, output)
            losses.total_loss.backward()
            self.optimizer.step()

            upto_log_time.update(time.time() - end)
            if self.log_outputs_now:
                self.model.module.log_outputs(output, inputs, losses, self.global_step,
                                       phase='train')
    ...
    def eval_for_dataset(self, val_loader, val_dataset):
        ...
        self.model_val.module.load_state_dict(self.model.module.state_dict())
        self.model_val.module.eval()
        self.model_val.module.throttle_log_images = 0  # make sure to log images the first val pass!
        self.model_val.module.set_dataset_sufix(val_dataset._hp)
        ...
        with autograd.no_grad():
            for batch_idx, sample_batched in enumerate(val_loader):
                ...
                losses = self.model_val.module.loss(inputs, output)
                losses_meter.update(losses)
                del losses

            self.model_val.module.log_outputs(
...
if __name__ == '__main__':
   ...
   # torch.distributed.init_process_group('nccl', init_method='env://',rank=0,world_size=4)
